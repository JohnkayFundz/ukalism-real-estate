...
function Autos() {
  ...
  const makes = useMemo(() => ['All Makes', ...new Set(autos.map((auto) => auto.make).filter(Boolean))], [])
  ...
  }), [query, make, condition, bodyType, transmission, fuel])
  ...
  const sendWhatsApp = (vehicle = selected) => {
    const message = vehicle
      ? `Hello Ukalism Autos, I am interested in the ${vehicle.title} in ${vehicle.location}. Please provide more information.`
      : 'Hello Ukalism Autos, I would like to enquire about available vehicles.'
    ...
    return (
      <div className="autos-page autos-details-page">
        <header className="autos-header">
          <button className="autos-brand" onClick={() => setSelected(null)}><span>U</span><strong>UKALISM</strong><small>AUTOS</small></button>
        ...
        <main className="autos-details-shell">
          ...
          <div className="vehicle-gallery">
            <div className="vehicle-hero-image">
              {images[0] && <img src={images[currentImage]} alt={selected.title} />}
              {selected.video_url && <span className="video-pill"><Play size={14} /> Video tour</span>}
              {images.length > 1 && <><button onClick={() => setCurrentImage((currentImage - 1 + images.length) % images.length)} className="gallery-nav left"><ArrowLeft size={20} /></button><button onClick={() => setCurrentImage((currentImage + 1) % images.length)} className="gallery-nav right"><ArrowRight size={20} /></button></>}
            </div>
            {images.length > 1 && <div className="vehicle-thumbs">{images.map((image, index) => <button className={index === currentImage ? 'active' : ''} key={image} onClick={() => setCurrentImage(index)}><img src={image} alt={`${selected.title} ${index + 1}`} /></button>)}</div>}
          </div>
          <div className="vehicle-details-grid">
            ...
            <aside className="vehicle-contact-card"><div className="contact-card-icon"><Gauge size={24} /></div><h2>Interested in this vehicle?</h2><p>Contact Ukalism Autos for availability, additional photos, inspection details and next steps.</p><button onClick={() => sendWhatsApp()} className="autos-primary">WhatsApp Seller <ArrowRight size={17} /></button><a href="tel:+2349041367161" className="autos-secondary">Call +234 904 136 7161</a></aside>
          </div>
        </main>
      </div>
    )
  ...
  return (
    <div className="autos-page">
      <header className="autos-header">
        <a className="autos-brand" href="/"><span>U</span><strong>UKALISM</strong><small>AUTOS</small></a>
      ...
      <main>
        <section className="autos-hero">
          <div className="autos-hero-content">
            <span className="autos-eyebrow">Ukalism Marketplace</span>
            <h1>Find Your Next Vehicle With Confidence.</h1>
            <p>Browse cars and other vehicles by make, model, year, condition, mileage and key specifications.</p>
          </div>
        </section>
        <section className="autos-search-wrap">
          <div className="autos-search-bar"><Search size={20} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search make, model or vehicle..." /></div>
          <div className="autos-filter-grid">
